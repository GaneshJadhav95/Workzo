...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
	if (isset($_POST['name']) && isset($_POST['email']) && isset($_POST['contact']) && isset($_FILES['profile_p']) && isset($_POST['password']) && isset($_POST['city']) && isset($_POST['state']) && isset($_POST['country']) && isset($_POST['company']) && isset($_POST['about']) && isset($_POST['servises'])) {
		...
		$check = $conn->prepare("SELECT * FROM `client` WHERE `email` = ? OR `contact` = ?");
			...
			if (move_uploaded_file($fileTmp, $destination)) {
				if ($check1->num_rows > 0) {
					...
					if (empty($errors)) {
						...
						$sql = $conn->prepare("INSERT INTO `client`(`name`, `email`, `contact`, `profile_p`, `password`, `city`, `state`, `country`, `company`, `about`, `servises`) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
						$sql->bind_param("ssissssssss", $name, $email, $contact, $newName, $password, $city, $state, $country, $company, $about, $servises);
						$sql->execute();
						if ($sql) {
							echo "<script>
